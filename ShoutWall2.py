...
class ShoutLog:
    ...
    def write_test_entry(self):
        try:
            ...
            log.write("[Test] " + self.timestamp() + " " + raw_input("Value of test: ") + " \"" +
                      raw_input("Description of test: ") + "\"\n")
            self.tests += 1
    ...
    def finalize_combined(self, weeks_back=1):
        ...
        if fin and not raw_input("One or more of the logs is already finalized.\nContinue?  ").upper().startswith("Y"):
    ...
    def process_candi(self, candi):
        ...
        if len(candi) > 0 and candi[0] == '/':
            if candi[1:5] == "help":
                ...
                number_lost = int(raw_input("Insert how many lost shouts?\n"))
                self.insert_lost_shouts(number_lost)

            elif candi[1:5] == "addu":
                self.write_username(raw_input("Username to add: "))
                ...
                if len(candi) > 1:
                    try:
                        ...
                        wb = int(raw_input("How many weeks back do you want to read? "))
...
if __name__ == "__main__":
    print("**************************************************************\n" +
          "Shout Wall Log Python Model 3.0")
    ...
    cmd = raw_input("<{:03d}>  ".format(shoutLog.shouts + 1))
    while cmd != "/done":
